...
def create_app(config_name=None):
    ...
    def runtime_diagnostics():
        import sys, platform, shutil, subprocess, glob
        from services.document_preview_service import find_libreoffice_executable
        ...
        candidates.extend(glob.glob("/usr/lib/libreoffice*/program/soffice"))
        candidates.extend(glob.glob("/opt/libreoffice*/program/soffice"))
        candidates.extend(glob.glob(os.path.join(app.root_path, "libreoffice_bin", "**", "soffice"), recursive=True))

        checked_candidates = []
        for cand in candidates:
            exists = os.path.exists(cand)
            is_file = os.path.isfile(cand) if exists else False
            is_exec = (os.name == 'nt' or os.access(cand, os.X_OK)) if exists else False
        ...
        discovered_bin = find_libreoffice_executable()
        version_info = None
        if discovered_bin:
            try:
                vres = subprocess.run(
                    [discovered_bin, '--version'],
                    capture_output=True,
                    text=True,
                    timeout=10
                )
                version_info = {
                    ...
                    'stdout': vres.stdout.strip(),
                    'stderr': vres.stderr.strip()
                }
            except Exception as ve:
                version_info = {'error': str(ve)}
        ...
        if os.path.exists('/proc/1/cgroup'):
            try:
                with open('/proc/1/cgroup', 'r') as f:
                    cgroup_docker = 'docker' in f.read()
            except Exception:
                pass
        ...
        return jsonify({
            ...
            'libreoffice': {
                'shutil_which_soffice': shutil.which('soffice'),
                'shutil_which_libreoffice': shutil.which('libreoffice'),
                'discovered_binary': discovered_bin,
                'version_info': version_info,
                'checked_candidates': checked_candidates,
            },
            'environment_keys': sorted([k for k in os.environ.keys() if not any(s in k.lower() for s in ['key', 'secret', 'pass', 'token', 'auth'])])
        })
